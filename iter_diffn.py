...
#	value = ( prev+next*( exp(( next-phi )/0.026 )))/( 1+exp(( next-phi )/0.026 ) )
	value = ( prev+next*( exp(( next-prev )/0.026 )))/( 1+exp(( next-prev )/0.026 ) ) # when difference form taken directly #works better
#	value = ( ((next-prev)*(next-prev)) + (4.0*0.026*(next+prev)) )/(8.0*0.026) # when exponentials are removed
#	if abs(phi-value)>precision:
#		value = f(value,prev,next,precision)
	return value

err=precision*len(xarray)*3
while (err > precision*len(xarray)*2):
#for m in range(100):
	err=0
